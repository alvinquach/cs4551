...
public class DCTBlock {
	...
	/** Creates an DCT block with all zeros. */
	public DCTBlock() {
		values = new float[BLOCK_SIZE][BLOCK_SIZE];
	}
	
	/** Creates a DCT block from an image block. DCT values are automatically computed. */
	public DCTBlock(float[][] values) throws Exception {
			...
			for (int v = 0; v < BLOCK_SIZE; v++) {
				...
				for (int i = 0; i < BLOCK_SIZE; i++) {
					...
					for (int j = 0; j < BLOCK_SIZE; j++) {
						sum += cos * Math.cos((2 * j + 1) * v * Math.PI / (2 * BLOCK_SIZE)) * values[i][j];
					}
				}
				float value = c * sum; // Raw DCT value.
				
				// Clamp the value to between -2e10 and 2e10, and then assign it to the array.
				this.values[u][v] = MathUtils.clamp(value, DCT_MIN_VALUE, DCT_MAX_VALUE);
			}
		}
